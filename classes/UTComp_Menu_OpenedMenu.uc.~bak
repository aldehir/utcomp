...
function InitComponent(GUIController MyController, GUIComponent MyOwner)
{
   // local mutator mut;
    ...
    l_Mode[2].Caption="UTComp Version"$class'Gameinfo'.static.MakeColorCode(GoldColor)$" 1.7c";


  /*  for ( mut=PlayerOwner().Level.Game.BaseMutator; mut!=None; mut=mut.NextMutator )
	if ( mut.IsA('Forward_Mutator') )
	{
         i_UTCompLogo.Image = texture'UTCompv17c.ForwardLogo';
         return;
	}
  */
...
DefaultProperties
{
     ...
     ImageStyle=ISTY_Scaled
		...
		WinLeft=0.312500
		WinTop=0.307113
        Image=Texture'UTCompv17c.UTCompLogo'
     End Object
