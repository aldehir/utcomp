...
{
    bSoftRestart=true;
    if(class'UTCompv17c.MutUTComp'.default.bEnableWarmup || Level.Game.IsA('UTComp_ClanArena'))
    {
...
function Timer()
{
    if(bWaitingOnRestart)
    {
        ...
        if(RestartWaitTime <=0)
        {
           ...
           BroadCastLocalizedMessage(class'Silent_TimerMessage', RestartWaitTime);
        return;
    }
